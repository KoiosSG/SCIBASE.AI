# Structured Abstract Consistency Report

| Packet | Status | Author draft | AI peer review | Editor summary | Findings |
| --- | --- | --- | --- | --- | --- |
| blocked-packet.json | hold_peer_review_packet | revise_required | blocked | blocked | CONCLUSION_OVERSTATES_EVIDENCE, METHODS_DESIGN_MISMATCH, MISSING_LIMITATION_LANGUAGE, RESULT_DIRECTION_MISMATCH, SAMPLE_SIZE_MISMATCH, SAMPLE_SIZE_MISMATCH |
| revision-packet.json | stage_for_author_revision | revision_queue | draft_only | withhold | MISSING_ABSTRACT_SECTION |
| clean-packet.json | release_peer_review_packet | allowed | allowed | allowed | none |

All packets use synthetic manuscript records and deterministic SHA-256 audit digests.
The assistant runs before AI peer-review packets or editor summaries are shown.
