...
- Invalid checksum placeholders such as `pending` do not count as durable identifier or API snapshot evidence.
- Invalid DOI placeholders such as `pending` do not count as durable identifier evidence.
- Truncated checksum values such as `sha256:abcdef` do not count as API snapshot evidence.
